...
{
  state_publisher_thread_ = std::make_unique<StatePublisherThread>(*this);
...
void PlayThread::SetMeasurement(const std::shared_ptr<eCAL::experimental::measurement::base::ReaderOld>& measurement, const std::string& path)
{
  std::unique_ptr<MeasurementContainer> new_measurment_container;

  if (measurement)
  {
    new_measurment_container = std::make_unique<MeasurementContainer>(measurement, path);
  }
