...
{
  TCLAP::CmdLine cmd("eCAL Recorder", ' ', ECAL_REC_VERSION);
  ...
  std::string rec_name = std::string("eCAL Recorder");
  std::string rec_version = std::string("Version: ") + ECAL_REC_VERSION;
  ...
  std::shared_ptr<eCAL::pb::rec::EcalRecService> rec_service_(new EcalRecService(ecal_rec));
  eCAL::protobuf::CServiceServer<eCAL::pb::rec::EcalRecService> rec_service_server_(rec_service_);
...
bool recorderBusy()
{
  bool any_writer_running = false;
  auto recorder_state = ecal_rec->GetRecorderState();
  if (recorder_state.main_recorder_state_.recording_ || recorder_state.main_recorder_state_.flushing_)
  {
    any_writer_running = true;
  }
  else
  {
    for (const auto& buffer_writer : recorder_state.buffer_writers_)
    {
      if (buffer_writer.recording_ || buffer_writer.flushing_)
      {
        any_writer_running = true;
        break;
      }
