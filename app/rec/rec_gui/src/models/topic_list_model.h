...
public:
  ...
  struct Topic
  {
    QString name_;
    QString type_;
    std::set<std::string> publisher_hosts_;
    bool listed_;
  ...
  void reset(const std::map<std::string, eCAL::rec::TopicInfo>& topic_info_map, const std::set<std::string>& listed_topics, bool add_additional_listed_topics);
...
public slots:
  void updateVisibleTopics(const std::map<std::string, eCAL::rec::TopicInfo>& topic_info_map, bool add_new_entries);
...
private:
  ...
  {
    { Columns::NAME,              "Topics" } ,
    { Columns::HOSTS,             "Published by" } ,
    { Columns::RECORDING_ENABLED, "Recorded" } ,
  };
