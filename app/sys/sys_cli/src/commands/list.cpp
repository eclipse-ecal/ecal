...
namespace eCAL
{
  namespace sys
  {
    namespace command
    {
      ...
      eCAL::sys::Error List::Execute(const std::string& hostname, const std::shared_ptr<eCAL::protobuf::CServiceClientUntyped<eCAL::pb::sys::Service>>& remote_ecalsys_service, const std::vector<std::string>& argv) const
      {
        bool task_mode = false;
        bool group_mode = false;
        bool runner_mode = false;
