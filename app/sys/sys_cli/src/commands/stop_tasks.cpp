...
namespace eCAL
{
  namespace sys
  {
    namespace command
    {
      ...
      std::string StopTask::Usage() const
      {
        return "[IDs or names]";
      }

      std::string StopTask::Help() const
      {
        return "Stop tasks with the given IDs or names. If no ID or name is given, all tasks will be stopped.";
      }

      std::string StopTask::Example() const
      {
      ...
      eCAL::sys::Error StopTask::Execute(const std::shared_ptr<EcalSys>& ecalsys_instance, const std::vector<std::string>& argv) const
      {
        ...
        else
        {
          ...
          {
            ecalsys_instance->StopTaskList(task_list, true, true);
            return eCAL::sys::Error::ErrorCode::OK;
          }
        }
      }

      eCAL::sys::Error StopTask::Execute(const std::string& hostname, const std::shared_ptr<eCAL::protobuf::CServiceClientUntypedCallback<eCAL::pb::sys::Service>>& remote_ecalsys_service, const std::vector<std::string>& argv) const
      {
