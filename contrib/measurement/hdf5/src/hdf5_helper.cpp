...
hid_t CreateFile(const std::string& filename, const std::string& file_version)
{
  ...
  hid_t file_id_ = H5Fcreate(filename.c_str(), H5F_ACC_TRUNC, fileCreateProperty, fileAccessPropery);

  if (file_id_ >= 0)
    SetAttribute(file_id_, kFileVerAttrTitle, file_version.c_str());
...
void WriteBinaryDataset(hid_t root, uint64_t counter, const BinaryEntry& entry)
{
  ...
  auto dataSet = H5Dcreate(root, std::to_string(counter).c_str(), H5T_NATIVE_UCHAR, dataSpace, H5P_DEFAULT, dsProperty, H5P_DEFAULT);

  //  Write buffer to dataset
  herr_t writeStatus = H5Dwrite(dataSet, H5T_NATIVE_UCHAR, H5S_ALL, H5S_ALL, H5P_DEFAULT, entry.address);
...
bool CreateInformationEntryInRoot(hid_t root, const std::string& url, const EntryInfoVect& entries)
{
...
bool GetEntryInfoVector(hid_t root, const std::string& url, EntryInfoSet& entries)
{
  entries.clear();
  ...
  for (unsigned int index = 0; index < data_size; index += 5)
  {
    ...
    entries.emplace(EntryInfo(data[index], data[index + 1], data[index + 2], data[index + 3], data[index + 4]));
  }
...
void CreateChannelNamesAttribute(hid_t root, const std::set<std::string>& channel_names)
{
  ...
  std::string joined_names = std::accumulate(std::next(channel_names.begin()), channel_names.end(),
    *channel_names.begin(),
    [](std::string a, const std::string& b) {
      return std::move(a) + ',' + b;
    });

  SetAttribute(root, kChnAttrTitle, joined_names);
}

void v6::WriteChannelSummary(hid_t root, const Channel& channel, const ChannelSummary& summary)
{
  std::string hex_id = printHex(channel.id);

  // Create a group with the cannel name
  auto group_name_id = OpenOrCreateGroup(root, channel.name);
  auto group_id_id = OpenOrCreateGroup(group_name_id, hex_id);

  CreateStringEntryInRoot(group_id_id, kChnIdTypename, summary.datatype_info.name);
  CreateStringEntryInRoot(group_id_id, kChnIdEncoding, summary.datatype_info.encoding);
  CreateStringEntryInRoot(group_id_id, kChnIdDescriptor, summary.datatype_info.descriptor);
  CreateInformationEntryInRoot(group_id_id, kChnIdData, summary.entry_info);
#pragma message("Do we need to sort the entries before we write them to the file??")
