...
namespace eCAL
{
  namespace PublisherOptional
  {
    namespace SHM
    {
      struct Configuration
      {
        std::optional<bool>                                  enable;                   //!< enable layer
        std::optional<bool>                                  zero_copy_mode;           //!< enable zero copy shared memory transport mode
        std::optional<unsigned int>                          acknowledge_timeout_ms;   /*!< force connected subscribers to send acknowledge event after processing the message
                                                                                            the publisher send call is blocked on this event with this timeout (0 == no handshake) */
        std::optional<Types::ConstrainedInteger<4096, 4096>> memfile_min_size_bytes;   //!< default memory file size for new publisher
        std::optional<Types::ConstrainedInteger<50, 1, 100>> memfile_reserve_percent;  //!< dynamic file size reserve before recreating memory file if topic size changes
        std::optional<Types::ConstrainedInteger<1, 1>>       memfile_buffer_count;     //!< maximum number of used buffers (needs to be greater than 1, default = 1)
      };
    }
    ...
    namespace UDP
    {
      ...
      {
        std::optional<bool>                                      enable;               //!< enable layer
        std::optional<bool>                                      loopback;             //!< enable to receive udp messages on the same local machine
        std::optional<Types::ConstrainedInteger<5242880, 1024>>  sndbuf_size_bytes;    //!< udp send buffer size in bytes (default 5MB)
      };
    }
    ...
    struct Configuration
    {
      ...
      std::optional<SHM::Configuration>   shm;
      std::optional<UDP::Configuration>   udp;
      std::optional<TCP::Configuration>   tcp;

      std::optional<bool>                 share_topic_type;                            //!< share topic type via registration
      std::optional<bool>                 share_topic_description;                     //!< share topic description via registration
    };
  }
}
