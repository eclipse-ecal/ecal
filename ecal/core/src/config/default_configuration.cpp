...
namespace {
  ...
  {
    std::string result = "[";
    if (filter_ & log_level_all)     result += "\"all\", ";
    if (filter_ & log_level_info)    result += "\"info\", ";
    if (filter_ & log_level_warning) result += "\"warning\", ";
    if (filter_ & log_level_error)   result += "\"error\", ";
    if (filter_ & log_level_fatal)   result += "\"fatal\", ";
    if (filter_ & log_level_debug1)  result += "\"debug1\", ";
    if (filter_ & log_level_debug2)  result += "\"debug2\", ";
    if (filter_ & log_level_debug3)  result += "\"debug3\", ";
    if (filter_ & log_level_debug4)  result += "\"debug4\", ";
...
namespace eCAL
{
  namespace Config
  {
    ...
    {
      auto& cfg = eCAL::GetConfiguration();
      std::stringstream ss;
      ss << std::boolalpha;
      ...
      ss << R"(  registration_refresh: )"                            << cfg.registration.registration_refresh                   << "\n";
      ss << R"(  # Timeout for topic registration in ms (internal, Default: 60000))"                                            << "\n";
      ss << R"(  registration_timeout: )"                            << cfg.registration.registration_timeout                   << "\n";
      ss << R"(  # Enable to receive registration information on the same local machine)"                                       << "\n";
      ss << R"(  loopback: )"                                        << cfg.registration.loopback                               << "\n";
      ...
      ss << R"(  host_group_name: )"                                 << quoteString(cfg.registration.host_group_name)           << "\n";
      ss << R"(  # true  = all eCAL components communicate over network boundaries)"                                            << "\n";
      ss << R"(  # false = local host only communication (Default: false))"                                                     << "\n";
      ss << R"(  network_enabled: )"                                 << cfg.registration.network_enabled                        << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"(  layer:)"                                                                                                       << "\n";
      ss << R"(    shm:)"                                                                                                       << "\n";
      ss << R"(      enable: )"                                      << cfg.registration.layer.shm.enable                       << "\n";
      ss << R"(      # Domain name for shared memory based registration)"                                                       << "\n";
      ss << R"(      domain:  )"                                     << quoteString(cfg.registration.layer.shm.domain)          << "\n";
      ss << R"(      # Queue size of registration events)"                                                                      << "\n";
      ss << R"(      queue_size: )"                                  << cfg.registration.layer.shm.queue_size                   << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"(    udp:)"                                                                                                       << "\n";
      ss << R"(      enable: )"                                      << cfg.registration.layer.udp.enable                       << "\n";
      ss << R"(      port: )"                                        << cfg.registration.layer.udp.port                         << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"(# Monitoring configuration)"                                                                                     << "\n";
      ss << R"(monitoring:)"                                                                                                    << "\n";
      ss << R"(  # Timeout for topic monitoring in ms (Default: 5000), increase in 1000er steps)"                               << "\n";
      ss << R"(  timeout: )"                                         << cfg.monitoring.timeout                                  << "\n";
      ss << R"(  # Topics blacklist as regular expression (will not be monitored))"                                             << "\n";
      ss << R"(  filter_excl: )"                                     << quoteString(cfg.monitoring.filter_excl)                 << "\n";
      ss << R"(  # Topics whitelist as regular expression (will be monitored only) (Default: ""))"                              << "\n";
      ss << R"(  filter_incl: )"                                     << quoteString(cfg.monitoring.filter_incl)                 << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"()"                                                                                                               << "\n";
      ...
      ss << R"(    config_version: )"                                << quoteString(cfg.transport_layer.udp.config_version)     << "\n";
      ss << R"(    # Valid modes: local, network (Default: local))"                                                             << "\n";
      ss << R"(    mode: )"                                          << quoteString(cfg.transport_layer.udp.mode)               << "\n";
      ss << R"(    # Multicast port number)"                                                                                    << "\n";
      ss << R"(    port: )"                                          << cfg.transport_layer.udp.port                            << "\n";
      ...
      ss << R"(    mask: )"                                          << quoteString(cfg.transport_layer.udp.mask)               << "\n";
      ss << R"(    # Send buffer in bytes)"                                                                                     << "\n";
      ss << R"(    send_buffer: )"                                   << cfg.transport_layer.udp.send_buffer                     << "\n";
      ss << R"(    # Receive buffer in bytes)"                                                                                  << "\n";
      ss << R"(    receive_buffer: )"                                << cfg.transport_layer.udp.receive_buffer                  << "\n";
      ...
      ss << R"(    join_all_interfaces: )"                           << cfg.transport_layer.udp.join_all_interfaces             << "\n";
      ss << R"(    # Windows specific setting to enable receiving UDP traffic with the Npcap based receiver)"                   << "\n";
      ss << R"(    npcap_enabled: )"                                 << cfg.transport_layer.udp.npcap_enabled                   << "\n";
      ss << R"()"                                                                                                               << "\n";
      ...
      ss << R"(    network:)"                                                                                                   << "\n";
      ss << R"(      # Multicast group base. All registration and logging is sent on this address)"                             << "\n";
      ss << R"(      group: )"                                       << quoteString(cfg.transport_layer.udp.network.group)      << "\n";
      ss << R"(      # TTL (hop limit) is used to determine the amount of routers being traversed towards the destination)"     << "\n";
      ss << R"(      ttl: )"                                         << cfg.transport_layer.udp.network.ttl                     << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"(  tcp: )"                                                                                                        << "\n";
      ss << R"(    # Reader amount of threads that shall execute workload)"                                                     << "\n";
      ss << R"(    number_executor_reader: )"                        << cfg.transport_layer.tcp.number_executor_reader          << "\n";
      ss << R"(    # Writer amount of threads that shall execute workload)"                                                     << "\n";
      ss << R"(    number_executor_writer: )"                        << cfg.transport_layer.tcp.number_executor_writer          << "\n";
      ss << R"(    # Reconnection attemps the session will try to reconnect in case of an issue)"                               << "\n";
      ss << R"(    max_reconnections: )"                             << cfg.transport_layer.tcp.max_reconnections               << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"(  shm:)"                                                                                                         << "\n";
      ss << R"(    # Default memory file size for new publisher)"                                                               << "\n";
      ss << R"(    memfile_min_size_bytes: )"                        << cfg.transport_layer.shm.memfile_min_size_bytes          << "\n";
      ss << R"(    # Dynamic file size reserve before recreating memory file if topic size changes)"                            << "\n";
      ss << R"(    memfile_reserve_percent: )"                       << cfg.transport_layer.shm.memfile_reserve_percent         << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"()"                                                                                                               << "\n";
      ...
      ss << R"(    shm:)"                                                                                                       << "\n";
      ss << R"(      # Enable layer)"                                                                                           << "\n";
      ss << R"(      enable: )"                                      << cfg.publisher.layer.shm.enable                          << "\n";
      ss << R"(      # Enable zero copy shared memory transport mode)"                                                          << "\n";
      ss << R"(      zero_copy_mode: )"                              << cfg.publisher.layer.shm.zero_copy_mode                  << "\n";
      ...
      ss << R"(      acknowledge_timeout_ms: )"                      << cfg.publisher.layer.shm.acknowledge_timeout_ms          << "\n";
      ss << R"(      # Maximum number of used buffers (needs to be greater than 1, default = 1))"                               << "\n";
      ss << R"(      memfile_buffer_count: )"                        << cfg.publisher.layer.shm.memfile_buffer_count            << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"(    # Base configuration for UDP publisher)"                                                                     << "\n";
      ss << R"(    udp:)"                                                                                                       << "\n";
      ss << R"(      # Enable layer)"                                                                                           << "\n";
      ss << R"(      enable: )"                                      << cfg.publisher.layer.udp.enable                          << "\n";
      ss << R"()"                                                                                                               << "\n";
      ...
      ss << R"(  share_topic_type: )"                                << cfg.publisher.share_topic_type                          << "\n";
      ss << R"(  # Share topic description via registration)"                                                                   << "\n";
      ss << R"(  share_topic_description: )"                         << cfg.publisher.share_topic_description                   << "\n";
      ss << R"(  # Priority list for layer usage in local mode (Default: SHM > UDP > TCP))"                                     << "\n";
      ss << R"(  priority_local: )"                                  << quoteString(cfg.publisher.layer_priority_local)         << "\n";
      ss << R"(  # Priority list for layer usage in cloud mode (Default: UDP > TCP))"                                           << "\n";
      ss << R"(  priority_network: )"                                << quoteString(cfg.publisher.layer_priority_remote)        << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"()"                                                                                                               << "\n";
      ...
      ss << R"(    shm:)"                                                                                                       << "\n";
      ss << R"(      # Enable layer)"                                                                                           << "\n";
      ss << R"(      enable: )"                                        << cfg.subscriber.layer.shm.enable                       << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"(    # Base configuration for UDP subscriber)"                                                                    << "\n";
      ss << R"(    udp:)"                                                                                                       << "\n";
      ss << R"(      # Enabler layer)"                                                                                          << "\n";
      ss << R"(      enable: )"                                        << cfg.subscriber.layer.udp.enable                       << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"(    # Base configuration for TCP subscriber)"                                                                    << "\n";
      ss << R"(    tcp:)"                                                                                                       << "\n";
      ss << R"(      # Enable layer)"                                                                                           << "\n";
      ss << R"(      enable: )"                                        << cfg.subscriber.layer.tcp.enable                       << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"(  # Enable dropping of payload messages that arrive out of order)"                                               << "\n";
      ss << R"(  drop_out_of_order_messages: )"                        << cfg.subscriber.drop_out_of_order_messages             << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"()"                                                                                                               << "\n";
      ...
      ss << R"(  rt: )"                                               << quoteString(cfg.timesync.timesync_module_rt)           << "\n";
      ss << R"(  # Specify the module name for replaying)"                                                                      << "\n";
      ss << R"(  replay: )"                                           << quoteString(cfg.timesync.timesync_module_replay)       << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"(# Service configuration)"                                                                                        << "\n";
      ss << R"(service:)"                                                                                                       << "\n";
      ss << R"(  # Support service protocol v0, eCAL 5.11 and older)"                                                           << "\n";
      ss << R"(  protocol_v0: )"                                      << cfg.service.protocol_v0                                << "\n";
      ss << R"(  # Support service protocol v1, eCAL 5.12 and newer)"                                                           << "\n";
      ss << R"(  protocol_v1: )"                                      << cfg.service.protocol_v1                                << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"()"                                                                                                               << "\n";
      ...
      ss << R"(    filter_excl: )"                                    << quoteString(cfg.application.sys.filter_excl)           << "\n";
      ...
      ss << R"(    emulator: )"                                       << quoteString(cfg.application.startup.terminal_emulator) << "\n";
      ss << R"()"                                                                                                               << "\n";
      ss << R"()"                                                                                                               << "\n";
      ...
      ss << R"(      enable: )"                                       << cfg.logging.sinks.console.enable                       << "\n";
      ss << R"(      # Log level for console output)"                                                                           << "\n";
      ss << R"(      level: )"                                        << logToArray(cfg.logging.sinks.console.filter_log_con)   << "\n";
      ss << R"(    # File logging configuration)"                                                                               << "\n";
      ss << R"(    file:)"                                                                                                      << "\n";
      ss << R"(      # Enable file logging)"                                                                                    << "\n";
      ss << R"(      enable: )"                                       << cfg.logging.sinks.file.enable                          << "\n";
      ss << R"(      # Log level for file output)"                                                                              << "\n";
      ss << R"(      level: )"                                        << logToArray(cfg.logging.sinks.file.filter_log_file)     << "\n";
      ss << R"(      # Log file path)"                                                                                          << "\n";
      ss << R"(      path: )"                                         << quoteString(cfg.logging.sinks.file.path)               << "\n";
      ss << R"(    # UDP logging configuration)"                                                                                << "\n";
      ss << R"(    udp:)"                                                                                                       << "\n";
      ss << R"(      # Enable UDP logging)"                                                                                     << "\n";
      ss << R"(      enable: )"                                       << cfg.logging.sinks.udp.enable                           << "\n";
      ss << R"(      # Log level for UDP output)"                                                                               << "\n";
      ss << R"(      level:  )"                                       << logToArray(cfg.logging.sinks.udp.filter_log_udp)       << "\n";
      ss << R"(      # UDP)"                                                                                                    << "\n";
      ss << R"(      port: )"                                         << cfg.logging.sinks.udp.port                             << "\n";
      ss << R"()"                                                                                                               << "\n";
