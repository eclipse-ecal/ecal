...
namespace eCAL
{
  ...
  bool CDataWriter::Create(const std::string& topic_name_, const std::string& topic_type_, const std::string& topic_desc_)
  {
    ...
    // create udp unicast layer
    SetUseUdpUC(m_use_udp_uc);
  ...
  bool CDataWriter::Destroy()
  {
    ...
    // destroy udp unicast writer
    m_writer_udp_uc.Destroy();

    // destroy memory file writer
    m_writer_shm.Destroy();

    // destroy lcm writer
    m_writer_lcm.Destroy();
  ...
  size_t CDataWriter::Send(const void* const buf_, size_t len_, long long time_, long long id_)
  {
    ...
    TLayer::eSendMode use_udp_uc(m_use_udp_uc);
    TLayer::eSendMode use_shm(m_use_shm);
    TLayer::eSendMode use_lcm(m_use_lcm);
#ifdef ECAL_LAYER_FASTRTPS
    ...
    // LAYER 4 : UDP (UC)
    ////////////////////////////////////////////////////////////////////////////
    if (use_udp_uc == TLayer::smode_on)
    {
#ifndef NDEBUG
      // log it
      Logging::Log(log_level_debug3, m_topic_name + "::CDataWriter::Send::UDP_UC");
#endif

      // prepare send
      bool prepared(false);
      prepared = m_writer_udp_uc.PrepareSend(len_);
      ...
      size_t udp_uc_sent(0);
      {
        ...
        wdata.bandwidth = m_bandwidth_max_udp;
        wdata.loopback  = loopback;
        udp_uc_sent     = m_writer_udp_uc.Send(wdata);
        m_use_udp_uc_confirmed = true;
      }
      written |= udp_uc_sent > 0;

#ifndef NDEBUG
      // log it
      if (udp_uc_sent > 0)
      {
        Logging::Log(log_level_debug3, m_topic_name + "::CDataWriter::Send::UDP_UC - SUCCESS");
      }
      else
      {
        Logging::Log(log_level_error, m_topic_name + "::CDataWriter::Send::UDP_UC - FAILED");
      }
#endif
    }
    ...
    // LAYER 5 : LCM
    ////////////////////////////////////////////////////////////////////////////
    if (use_lcm == TLayer::smode_on)
    {
#ifndef NDEBUG
      // log it
      Logging::Log(log_level_debug3, m_topic_name + "::CDataWriter::Send::LCM");
#endif

      // prepare send
      if (m_writer_lcm.PrepareSend(len_))
      {
        // register new to update listening subscribers
        DoRegister(true);
        // let's rematch writer / reader
      ...
      size_t lcm_sent(0);
      {
        ...
        wdata.hash  = snd_hash;
        wdata.time  = time_;
        lcm_sent    = m_writer_lcm.Send(wdata) > 0;
        m_use_lcm_confirmed = true;
      }
      written |= lcm_sent > 0;
      ...
      if (lcm_sent > 0)
      {
        Logging::Log(log_level_debug3, m_topic_name + "::CDataWriter::Send::LCM - SUCCESS");
      }
      else
      {
        Logging::Log(log_level_error, m_topic_name + "::CDataWriter::Send::LCM - FAILED");
      }
#endif
    }
    ...
    // LAYER 6 : FASTRTPS 
    ////////////////////////////////////////////////////////////////////////////
    if (use_rtps == TLayer::smode_on)
    {
#ifndef NDEBUG
      // log it
      Logging::Log(log_level_debug3, m_topic_name + "::CDataWriter::Send::RTPS");
#endif

      // prepare send
      if (m_writer_rtps.PrepareSend(len_))
      {
        // register new to update listening subscribers
        DoRegister(true);
        // let's rematch writer / reader
      ...
      size_t rtps_sent(0);
      {
        ...
        wdata.hash  = snd_hash;
        wdata.time  = time_;
        rtps_sent   = m_writer_rtps.Send(wdata) > 0;
        m_use_rtps_confirmed = true;
      }
      written |= rtps_sent > 0;
      ...
      if (rtps_sent > 0)
      {
        Logging::Log(log_level_debug3, m_topic_name + "::CDataWriter::Send::RTPS - SUCCESS");
      }
      else
      {
        Logging::Log(log_level_error, m_topic_name + "::CDataWriter::Send::RTPS - FAILED");
      }
#endif
    }
#endif /* ECAL_LAYER_FASTRTPS */
  ...
  void CDataWriter::RefreshRegistration()
  {
    ...
    std::shared_ptr<std::list<std::string>> loc_timeouts = std::shared_ptr<std::list<std::string>>(new std::list<std::string>);
    {
      std::lock_guard<std::mutex> lock(m_sub_map_sync);
  ...
  bool CDataWriter::SetUseUdpUC(TLayer::eSendMode mode_)
  {
    m_use_udp_uc = mode_;
    if (!m_created) return true;

    // log send mode
    LogSendMode(m_use_udp_uc, m_topic_name + "::CDataWriter::Create::UDP_UC_SENDMODE::");

    switch (m_use_udp_uc)
    {
    case TLayer::eSendMode::smode_auto:
    case TLayer::eSendMode::smode_on:
      m_writer_udp_uc.Create(m_host_name, m_topic_name, m_topic_id);
#ifndef NDEBUG
      Logging::Log(log_level_debug4, m_topic_name + "::CDataWriter::Create::UDP_UC_WRITER");
#endif
      break;
    case TLayer::eSendMode::smode_none:
    case TLayer::eSendMode::smode_off:
  ...
  bool CDataWriter::SetUseLcm(TLayer::eSendMode mode_)
  {
    m_use_lcm = mode_;
    if (!m_created) return true;

    // log send mode
    LogSendMode(m_use_lcm, m_topic_name + "::CDataWriter::Create::LCM_SENDMODE::");

    switch (m_use_lcm)
    {
    case TLayer::eSendMode::smode_on:
      m_writer_lcm.Create(m_host_name, m_topic_name, m_topic_id);
#ifndef NDEBUG
      Logging::Log(log_level_debug4, m_topic_name + "::CDataWriter::Create::LCM_WRITER");
#endif
      break;
    default:
      m_writer_lcm.Destroy();
...
#ifdef ECAL_LAYER_FASTRTPS
  bool CDataWriter::SetUseRtps(TLayer::eSendMode mode_)
  {
    m_use_rtps = mode_;
    if (!m_created) return true;

    // log send mode
    LogSendMode(m_use_rtps, m_topic_name + "::CDataWriter::Create::RTPS_SENDMODE::");

    switch (m_use_rtps)
    {
    case TLayer::eSendMode::smode_on:
      m_writer_rtps.Create(m_host_name, m_topic_name, m_topic_id);
#ifndef NDEBUG
      Logging::Log(log_level_debug4, m_topic_name + "::CDataWriter::Create::RTPS_WRITER");
#endif
      break;
    default:
      m_writer_rtps.Destroy();
