...
namespace eCAL
{
  CDataWriterUdpUC::~CDataWriterUdpUC()
  {
    Destroy();
  }

  void CDataWriterUdpUC::GetInfo(SWriterInfo info_)
  {
    info_.name                 = "udp";
    info_.description          = "UDP unicast data writer";
  ...
  bool CDataWriterUdpUC::Create(const std::string & host_name_, const std::string & topic_name_, const std::string & topic_id_)
  {
    ...
    m_udp_ipaddr  = eCALPAR(NET, UDP_UNICAST_IPADDR);

    SSenderAttr attr;
    attr.ipaddr     = m_udp_ipaddr;
    attr.port       = eCALPAR(NET, UDP_UNICAST_PORT);
    attr.unicast    = true;
  ...
  size_t CDataWriterUdpUC::Send(const SWriterData& data_)
  {
    ...
    layer->set_type(eCAL::pb::eTLayerType::tl_ecal_udp_uc);
