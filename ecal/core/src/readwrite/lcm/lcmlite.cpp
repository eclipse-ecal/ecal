...
// packets.)
static void deliver_packet(lcmlite_sub_t *lcm, const char *channel, const void *buf, int buf_len)
{
...
{
    memset(lcm, 0, sizeof(lcmlite_sub_t));
    return 0;
}
...
{
  memset(lcm, 0, sizeof(lcmlite_pub_t));
...
int lcmlite_publish(lcmlite_pub *lcm, const char *channel, const void *_buf, int buf_len)
{
    if (buf_len < LCM_PUBLISH_BUFFER_SIZE - MAXIMUM_HEADER_LENGTH) {
        ...
        while (fragment_offset < (uint32_t)buf_len) {
            ...
            encode_u16(&lcm->publish_buffer[buf_pos], (uint16_t)fragment_id);      buf_pos += 2;
            encode_u16(&lcm->publish_buffer[buf_pos], (uint16_t)fragments_in_msg); buf_pos += 2;
