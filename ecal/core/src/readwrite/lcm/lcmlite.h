...
//
#define LCM3_NUM_BUFFERS         8              // increased from 4 to 8
#define LCM3_MAX_PACKET_SIZE    (4*1024*1024)   // increased from ca. 300 kB to 4 MB
#define LCM3_MAX_FRAGMENTS       512            // increased from 256 to 512
...
typedef struct lcmlite_sub lcmlite_sub_t;
typedef struct lcmlite_pub lcmlite_pub_t;
...
struct lcmlite_subscription
{
    ...
    void (*callback)(lcmlite_sub_t *lcm, const char *channel, const void *buf, int buf_len, void *user);
    void *user;
...
int lcmlite_sub_init(lcmlite_sub_t *lcm);

// Caller allocates the lcmlite_t object, which we initialize.
int lcmlite_pub_init(lcmlite_pub_t *lcm, void(*transmit_packet)(const void *_buf, int buf_len, void *user), void *transmit_user);
...
int lcmlite_receive_packet(lcmlite_sub_t *lcm, const void *_buf, int buf_len, uint64_t from_addr);
...
int lcmlite_publish(lcmlite_pub *lcm, const char *channel, const void *_buf, int buf_len);
...
void lcmlite_subscribe(lcmlite_sub_t *lcm, lcmlite_subscription_t *sub);
