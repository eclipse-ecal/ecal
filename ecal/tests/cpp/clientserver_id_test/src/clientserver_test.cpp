...
TEST(core_cpp_clientserver_id, ClientServerBaseAsyncCallback)
{
  const int calls(1);
  const int sleep(100);
  ...
  eCAL::CServiceClientID client("service");
...
TEST(core_cpp_clientserver_id, ClientServerBaseAsync)
{
  const int calls(5);
...
TEST(core_cpp_clientserver_id, NestedRPCCall)
{
  const int calls(1);
  const int sleep(0);
