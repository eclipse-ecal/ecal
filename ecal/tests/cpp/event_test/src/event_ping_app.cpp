...
int main(int argc, char** argv)
{
  ...
  if (!eCAL::gOpenNamedEvent(&ping_event, ping_event_name, true))
  {
    std::cerr << "Failed to open ping event." << std::endl;
    return 1;
  }

  if (!eCAL::gOpenNamedEvent(&pong_event, pong_event_name, true))
  {
  ...
  std::cout << "ping app ready. Iterations: " << iterations << std::endl;
  ...
  {
    if (!eCAL::gSetEvent(ping_event))
    {
      std::cerr << "Failed to set ping at iteration " << i << "." << std::endl;
      eCAL::gCloseEvent(pong_event);
      eCAL::gCloseEvent(ping_event);
      return 1;
    }

    if (!eCAL::gWaitForEvent(pong_event, wait_timeout_ms))
    {
      std::cerr << "Timeout waiting for pong at iteration " << i << "." << std::endl;
      eCAL::gCloseEvent(pong_event);
      eCAL::gCloseEvent(ping_event);
      return 2;
    }
