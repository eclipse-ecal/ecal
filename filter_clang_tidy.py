...
DEBUG = False
# If the commands JSON file is missing, treat this either as a normal program termination
# or as an abnormal program termination.
SKIP = False
...
def filter_commands():
    ...
    for command in commands:
        if DEBUG:
            print(command['file'])
        path_normalized = os.path.normpath(command['file'])
        path_dirname = os.path.dirname(path_normalized)
        # use relative path to remove the common part of the path and increase speed & reliability
        path_relative_to = os.path.relpath(path_dirname, PATH_RELATIVE)
        # path string starts artificially with path separator to prevent partial matches
        path_str = os.path.sep + str(path_relative_to) + os.path.sep
    ...
    if REPLACE_WITH_SYSTEM_HEADERS:
        ...
        for command in commands_inc:
            for exclude in excludes:
                ...
                header_prefix = f'-I{exclude_path}'
                header_replace = f'-isystem {exclude_path}'
                if header_prefix in command['command']:
                    if DEBUG:
                        print(f"-- {command['command']}")
                    command['command'] = command['command'].replace(header_prefix, header_replace)
                    if DEBUG:
                        print(f"++ {command['command']}")
