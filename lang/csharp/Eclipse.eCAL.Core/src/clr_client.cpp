...
// Constructor
ServiceClient::ServiceClient(String^ serviceName)
{
  // Convert serviceName to std::string.
  std::string nativeServiceName = StringToStlString(serviceName);
  // Initialize the native service client with the service name only.
  m_native_service_client = new ::eCAL::CServiceClient(nativeServiceName);
}
...
// Blocking call that returns a List<ServiceResponse^>.
