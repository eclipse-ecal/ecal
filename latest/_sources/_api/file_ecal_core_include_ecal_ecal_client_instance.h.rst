
.. _file_ecal_core_include_ecal_ecal_client_instance.h:

File ecal_client_instance.h
===========================


:ref:`namespace_eCAL` client instance interface 



.. contents:: Contents
   :local:
   :backlinks: none

Definition (``ecal/core/include/ecal/ecal_client_instance.h``)
--------------------------------------------------------------


.. toctree::
   :maxdepth: 1

   program_listing_file_ecal_core_include_ecal_ecal_client_instance.h.rst





Includes
--------


- ``ecal/ecal_callback.h`` (:ref:`file_ecal_core_include_ecal_ecal_callback.h`)

- ``ecal/ecal_deprecate.h`` (:ref:`file_ecal_core_include_ecal_ecal_deprecate.h`)

- ``ecal/ecal_os.h`` (:ref:`file_ecal_core_include_ecal_ecal_os.h`)

- ``ecal/ecal_service_info.h`` (:ref:`file_ecal_core_include_ecal_ecal_service_info.h`)

- ``ecal/ecal_types.h`` (:ref:`file_ecal_core_include_ecal_ecal_types.h`)

- ``memory``

- ``string``



Included By
-----------


- :ref:`file_ecal_core_include_ecal_ecal_client.h`




Namespaces
----------


- :ref:`namespace_eCAL`


Classes
-------


- :ref:`exhale_class_classeCAL_1_1CClientInstance`

