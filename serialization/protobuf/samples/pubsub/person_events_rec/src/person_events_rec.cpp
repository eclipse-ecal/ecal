...
void OnEvent(const char* topic_name_, const struct eCAL::v5::SSubEventCallbackData* data_)
{
  ...
  switch (data_->type)
  {
  case eCAL::eSubscriberEvent::sub_event_connected:
    std::cout << "event            : " << "sub_event_connected" << std::endl;
    break;
  case eCAL::eSubscriberEvent::sub_event_disconnected:
    std::cout << "event            : " << "sub_event_disconnected" << std::endl;
    break;
  case eCAL::eSubscriberEvent::sub_event_dropped:
    std::cout << "event            : " << "sub_event_dropped (" << data_->clock << " messages)" << std::endl;
    break;
  // not implemented yet
  case eCAL::eSubscriberEvent::sub_event_corrupted:
    std::cout << "event            : " << "sub_event_corrupted" << std::endl;
    break;
  case eCAL::eSubscriberEvent::sub_event_update_connection:
...
int main()
{
  ...
  sub.AddEventCallback(eCAL::eSubscriberEvent::sub_event_connected,         evt_callback);
  sub.AddEventCallback(eCAL::eSubscriberEvent::sub_event_disconnected,      evt_callback);
  sub.AddEventCallback(eCAL::eSubscriberEvent::sub_event_dropped,           evt_callback);
  sub.AddEventCallback(eCAL::eSubscriberEvent::sub_event_corrupted,         evt_callback);
  sub.AddEventCallback(eCAL::eSubscriberEvent::sub_event_update_connection, evt_callback);
